...
create_lustre_fs () {
	...
	then
		...
		echo " - mkfs.lustre --fsname=lustre --index=${INDEX} --ost --mgsnode=10.2.20.10@o2ib:10.2.20.11@o2ib --mkfsoptions=\"-E stride=262144,stripe_width=262144 -i 8192 ${JOURNAL}\" ${DEVICE} " >> $LOKI
		/usr/sbin/mkfs.lustre --fsname=lustre --index=${INDEX} --ost --mgsnode=10.2.20.10@o2ib:10.2.20.11@o2ib --mkfsoptions="-E stride=262144,stripe_width=262144 -i 8192 ${JOURNAL}" ${DEVICE}
	fi
	return 2
}
if [[ "$3" != "No" ]] ; then 
